	...
	<div class="col-lg-9 col-sm-9 col-md-9 col-xs-12">
		...
		<div class="panel panel-default visible-lg visible-md visible-sm hidden-xs">
			...
			<div class="panel-body">
					...
					<tbody>
						...
						@if(count($script))
							...
							<tr>
								<td>{{$no++}}</td>
								<td>
									<a href="{{url('/comic').'/'.$data->comic_title}}" title="{{str_replace('-', ' ', $data->comic_title)}}">{{str_replace('-', ' ', $data->comic_title)}}</a>
								</td>
								<td>{{str_replace('-', ' ', $data->comic_author)}}</td>
								<td>{{$data->comic_status}}</td>
							</tr>
							@endforeach
						@else
							<tr>
				...
				{{ $script->links('pagination.custom') }}
			</div>
		</div>
